...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ancestorsA = {}
        distanceA = 0
        parent = nodeA.get_parent()
        
        while parent != None:
            ancestorsA[parent] = distanceA
            parent = parent.get_parent()
            distanceA += 1

        parent = nodeB.get_parent()
        distanceB = 0
        
        if nodeB in ancestorsA:
            return (-1, ancestorsA[nodeB] + 1)
        
        while parent != None:
            if parent == nodeA:
                return (-1, distanceB + 1)
            if parent in ancestorsA:
                distanceA = ancestorsA[parent]
                break
            parent = parent.get_parent()         
            distanceB += 1
        if distanceB > distanceA:
            return (distanceA, distanceB - distanceA)
        else:
            return (distanceB, distanceA - distanceB)
...
t, r = f.cousin("a", "q")
print "'a' is a", words[t], "cousin", r, "removed from 'q'"